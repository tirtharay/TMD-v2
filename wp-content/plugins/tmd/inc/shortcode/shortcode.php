...
class shortcode
{
    ...
    public function __construct()
    {
        ...
        $this->actorlistpage = 'actors-list'; // actor list page slug
        $this->searchpage = 'search-results'; // Search page slug.
    ...
    public function get_popular_actors()
    {
        ...
        // $apiURL = 'https://api.themoviedb.org/3/search/person?api_key=' .  $this->apikey;
        $defaultimage = $this->defaultimage;
        $imageURL = $this->imagePath;
    ...
    public function get_movies_list($atts)
    {
        ...
        if (isset($atts['type']) && $atts['type'] == 'sidebar') {
            ...
            $movieyear = isset($_GET['movieyear']) ? $_GET['movieyear'] : date('Y');
...
?>
        <div class="search-form">
            <form id="search-form" action="<?php echo $this->searchpage ?>">
                <div class="filters">
                    <input type="radio" id="movies" name="type" value="both" <?php echo $bothselected ?>>
                    <label for="movies">Both ( Movies and Actors )</label><br>
                ...
                <div>
                    <div class="search-input">
                        <input placeholder="Please Search for Movie or Actor or both" class="filter-dd" type="text" name="search" value="<?php echo $search ?>" required />
                    </div>
...
<?php
    ...
    public function get_search_results()
    {
        ...
        if (!empty($_GET)) {
            ...
            if (empty($search)) {
                return 'Please Search for Movie or Actor or both';
            }
            ...
            if ($type == 'movies' || $type == 'both') {
                ...
                if (!empty($search)) {
                    $search_text = str_replace(' ', '%20', $search);
                    $search_text = '&with_text_query=' . $search_text;
                }


                $curl = curl_init();
                curl_setopt_array($curl, array(
                    CURLOPT_URL => 'https://api.themoviedb.org/3/discover/movie?api_key=' . $this->apikey . '&region=' . $this->region . '&sort_by=original_title.asc&page=' . $paged  . $search_text,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_SSL_VERIFYPEER => false,
                    CURLOPT_ENCODING => '',
                    CURLOPT_MAXREDIRS => 10,
